...
if ([string]::IsNullOrWhiteSpace($OutputPath)) {
	$packagePath = Join-Path $workspaceRoot 'xmplayAccessibility-1.3.0.nvda-addon'
} else {
...
try {
	xgettext --language=Python --keyword=_ --from-code=UTF-8 --package-name=xmplayAccessibility --package-version=1.3.0 -o 'xmplayAccessibility.pot' 'addon\appModules\xmplay\__init__.py' 'addon\appModules\xmplay\dialogs.py' 'addon\globalPlugins\xmplayAccessibility\__init__.py' 'addon\globalPlugins\xmplayAccessibility\settingsPanel.py'
